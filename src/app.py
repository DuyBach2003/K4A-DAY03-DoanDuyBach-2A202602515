...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    # Bộ nhớ hội thoại: Action + Observation của mỗi lượt được nạp lại cho LLM ở lượt kế tiếp
    ...
    while step < MAX_ITERATIONS:
        ...
        # Gọi LLM với Native Tool Calling Specs và toàn bộ lịch sử suy luận
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action) -> thực thi qua MCP Server rồi lặp tiếp
            ...
            # Thực thi Tool qua MCP Server (JSON-RPC -> dispatch_tool_call)
            ...
            # Nạp Action + Observation vào lịch sử để LLM suy luận tiếp ở lượt kế tiếp
            messages.append({
                "role": "assistant",
                "tool_call": {"id": call_id, "name": tool_name, "arguments": arguments},
                "provider_data": llm_response.get("provider_data")
            })
            messages.append({"role": "tool", "tool_call_id": call_id, "name": tool_name, "content": obs_str})

        else:
            print(f"⚠️ [CHÚ Ý]: LLM trả về phản hồi không hợp lệ: {llm_response}")
            break
    else:
        # Hết MAX_ITERATIONS mà LLM vẫn chưa đưa ra Final Answer
        final_answer = f"Đã đạt giới hạn {MAX_ITERATIONS} vòng lặp ReAct nhưng chưa có câu trả lời cuối cùng."
        ...
        trace_logs.append({
            ...
            "action_type": "FINAL_ANSWER",
            "model": "n/a",
            "thought": "Dừng vòng lặp do vượt quá MAX_ITERATIONS.",
            "output": final_answer,
            "latency_ms": 0.0
        })
